...
class _MinDistCluster<T extends ClusterItemCustom> {
  final ClusterCustom<T> cluster;
  final double dist;

  _MinDistCluster(this.cluster, this.dist);
}

class MaxDistClustering<T extends ClusterItemCustom> {
  ///Complete list of points
  late List<T> dataset;

  List<ClusterCustom<T>> _cluster = [];
  ...
  List<ClusterCustom<T>> run(List<T> dataset, int zoomLevel) {
    ...
    for (T entry1 in dataset) {
      distMatrix.add([]);
      _cluster.add(ClusterCustom.fromItems([entry1]));
    }
    bool changed = true;
    while (changed) {
      changed = false;
      for (ClusterCustom<T> c in _cluster) {
        ...
        _cluster.add(ClusterCustom.fromClusters(minDistCluster.cluster, c));
        _cluster.remove(c);
  ...
  _MinDistCluster<T>? getClosestCluster(ClusterCustom cluster, int zoomLevel) {
    double minDist = 1000000000;
    ClusterCustom<T> minDistCluster = ClusterCustom.fromItems([]);
    for (ClusterCustom<T> c in _cluster) {
      ...
      if (tmp < minDist) {
        minDist = tmp;
        minDistCluster = ClusterCustom<T>.fromItems(c.items);
      }
    }
